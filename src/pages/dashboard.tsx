...
import { getPastRides } from 'apiSdk/past-rides';
import { PastRideInterface } from 'interfaces/past-ride';
...
function DashboardPage() {
  ...
  const pastRideStat = useSWR<PaginatedInterface<PastRideInterface>>(
    () => `/past-rides`,
    () => getPastRides({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Past Rides',
      result: pastRideStat?.data?.totalCount,
      isLoading: pastRideStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Vehicles',
